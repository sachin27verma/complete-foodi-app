...
const CartPage = () => {
    ...
    const handleIncrease = async (item) => {
        try {
            const response = await fetch(` https://complete-foodi-server-whx8.onrender.com/carts/${item._id}`, {
                method: "PUT",
                headers: {
    ...
    const handleDelete = (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(` https://complete-foodi-server-whx8.onrender.com/carts/${item._id}`)
                    .then(response => {
                        if (response) {
                            refetch();
